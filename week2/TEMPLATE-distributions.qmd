...
mko_clean %>% 
  ...
  geom_density_ridges(rel_min_height = 0.001)  # Set minimum height to show only relevant dark lines leading from plot
...
ggplot(data = mko_clean, aes(x = month_name, y = Temp_bot, fill = month_name))+
  geom_boxplot() +
...
ggplot(mko_clean, aes(x = month_name, y = Temp_bot, fill = month_name)) +
  ...
  geom_boxplot(width = 0.2, fill = "gray", color = "gray30", alpha = 0.5) +
  coord_flip()
