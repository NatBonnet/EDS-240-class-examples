...
download_d1_data("https://cn.dataone.org/cn/v2/resolve/https%3A%2F%2Fpasta.lternet.edu%2Fpackage%2Fdata%2Feml%2Fknb-lter-hbr%2F208%2F14%2F024b6acc5cb2e03a14fff5558bbffc0c", path = here::here("week4")) 
...
#......................add a smoothed line.......................
stream_chem_data |> 
  # Filter for subset of water years- keep values between 2014 and 2024
  filter(waterYr %in% c(2014:2024)) |>
  # Filter for a random site
  filter(site == "W5") |> 
  ggplot(aes(x = DOC, y = pH)) + 
  geom_point(alpha = 0.5) +
  # Defaults to method = "lowest" with less than 10,000 data points
  geom_smooth()
# Not appropriate for sparse data or data that doesn't have a clear trend
```
...
stream_chem_data |> 
  ...
  ggplot(aes(x = DOC, y = pH)) + 
  geom_point(alpha = 0.5) +
  # Adding standard error line is deceptive because we haven't tested this data for any assumptions, looked at residuals, etc
  geom_smooth(method = "lm", se = FALSE)
```

## Visualizing a third numeric variable- map to another visual element other than position and color-size
...
#Overexaggerate differences because radius will for example quadruple the area of a value that is only twice as large
stream_chem_data |> 
  filter(waterYr == 2024) |> 
  ggplot(aes(x = DOC, y = pH, color = site, size = Al_ICP)) + 
  geom_point(alpha = 0.5) +
...
ggplot(stream_chem_data, aes(x = SO4, y = pH)) +
  geom_hex() +
  scale_fill_viridis_c(option = "inferno") +
  # Adjust colorbar size and breaks if you want
