...
sui_severity <- ca_region |>
  ...
  # Turn sui_cat into a factor so they can be reordered 
...
heatmap_data <- ca_region |> 
  ...
  # ungroup so you can subset not grouped by subregion and year
  ungroup()

#.determine order of subregions based on highest avg SUI in 2015.
order_2015 <- heatmap_data %>% 
  filter(year == 2015) %>% 
  arrange(by = annual_mean_sui) %>% 
...
#| echo: false
#.........................create heatmap.........................
ggplot(data = heatmap_order, aes(x = year, y = subregion_name, fill = annual_mean_sui)) +
  geom_tile() +
  scale_fill_viridis_c() +
  scale_x_continuous(breaks = seq(2010, 2020, by = 2)) +
  labs(fill = "Annual Mean Surface Water-Supply and\nUse Index (SUI)") + # Wrap text for title
...
ggplot(sbc_subbasin_monthly) +
  ...
  geom_point(aes(x = mean_avail, y = month), 
             ...
             shape = "triangle", 
             stroke = 3) +
  geom_point(aes(x = mean_consum, y = month), color = "#9C7344", 
             size = 3,
             shape= 21,
             fill = "white", 
             stroke = 2
